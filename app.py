...
try:
    ...
    def get_topic_keywords(*args, **kwargs):
        return []
...
try:
    ...
    st.error("❌ 未找到数据文件 `bailu_clean.csv`，请确认 A 同学已输出文件并放置在当前目录。")
    st.stop()
...
with col1:
    ...
    if 'sentiment_label' in df.columns:
        ...
        st.warning("缺少 sentiment_label 列")

with col2:
    st.subheader("👥 粉丝 vs 路人态度对比")
    if 'is_fan' in df.columns and 'sentiment_label' in df.columns:
        fig_bar = create_fan_vs_public_bar(df)
        st.plotly_chart(fig_bar, use_container_width=True)
    else:
        st.warning("缺少 is_fan 或 sentiment_label 列")

# ----------------------------- 情感演化折线图 ---------------------------
st.subheader("📈 情感演化趋势")
if 'created_at_datetime' in df.columns and 'sentiment_label' in df.columns:
...
if 'text' in df.columns:
    ...
    st.info("文本列 text 不存在，词云功能跳过")

# ----------------------------- 中立亚型桑基图 ---------------------------
st.subheader("🔀 中立态度的流向与细分")
if 'neutral_subtype' in df.columns and 'sentiment_label' in df.columns:
    ...
    st.info("neutral_subtype 列缺失，跳过桑基图")

# ----------------------------- 中立亚型时间演化（堆叠面积图） -----------
st.subheader("📊 中立亚型随时间演化")
if 'neutral_subtype' in df.columns and 'created_at_datetime' in df.columns:
    ...
    st.info("neutral_subtype 或日期列缺失")
...
if all(os.path.exists(f) for f in required_files):
    ...
    st.info("📌 LDA 模型尚未训练，请先运行 `python lda_analysis.py` 生成模型文件（需要中立评论数据）。")
